...
public:
    ...
    int level = 50;
    int exp = 52000;
    string entity_name_str = "Chubby Cait";
    string sprite_idle = "chubby_cait_idl";
    string sprite_attack = "chubby_cait_atk";
    string sprite_hurt = "chubby_cait_hrt";
    string sprite_death = "chubby_cait_dth";
    string ai_script = "behavior_chubby_cait";
...
public:
    ChubbyCait() {
        ...
        sprites_data.emplace_back("attack", sprite_attack);
        sprites_data.emplace_back("hurt", sprite_hurt);
        sprites_data.emplace_back("death", sprite_death);

        // Initialize attacks
        {
            AttackInfo info;
            info.damage_type = "physical";
            info.target_type = "row";
            info.damage_mod = 0.75f;
            info.mod_type = "multiplicative";
            info.effect = "poison";
            info.effect_chance = 0.15f;
            attacks_data.emplace_back("multi", info);
        }
        {
            AttackInfo info;
            info.damage_type = "physical";
    ...
    int multi_attack() {
        AttackInfo info = get_attack_info("multi");
        int damage = (info.damage_type == "physical") ? p_attack : m_attack;
        return deal_damage(info.damage_type, info.target_type, damage, info.damage_mod, info.mod_type, info.effect, info.effect_chance);
    }
};
